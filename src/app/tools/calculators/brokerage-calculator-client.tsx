...
export default function BrokerageCalculatorClient({
  ...
  const compareBroker = useMemo(() => {
    return (
      brokers.find((broker) => broker.id === compareBrokerId) ?? brokers[1]
    );
          ...
          <div className="border-t border-border/50 pt-4 mt-2">
            <div className="flex items-start justify-between gap-4 font-bold text-base">
              ...
              <span className="shrink-0 text-red-600 dark:text-red-400">
                {formatCurrency(res.totalTaxesAndCharges)}
              </span>
            </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_auto]">
            ...
            <CardContent className="space-y-6 pt-6">
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                {isComparing ? (
                  <div className="space-y-2">
                    ...
                    <Select
                      value={compareBrokerId}
                      onValueChange={setCompareBrokerId}
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleInputChange("exchange", value)
                      }
                    >
                      <SelectTrigger className="h-11 w-full rounded-lg bg-background font-medium text-foreground">
              ...
              <div className="space-y-4">
                <div className="space-y-3">
                  ...
                  >
                    <TabsList
                      className="grid h-auto w-full gap-1 rounded-lg bg-muted/30 p-1"
                      style={{
                        gridTemplateColumns: `repeat(${category === "equity" ? 3 : 2}, minmax(0, 1fr))`,
                      }}
                    >
                      {category === "equity" && (
                        <>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="mt-3 text-2xl font-bold tabular-nums text-foreground">
                {formatCurrency(
                  (selectedBroker.dpChargeConfig?.brokerFee ?? 0) +
                    (selectedBroker.dpChargeConfig?.depositoryFee ?? 0),
                )}
              </div>
        ...
        <div
          className={`grid gap-4 ${isComparing ? "xl:w-[700px] xl:grid-cols-2" : "xl:w-[380px]"}`}
        >
          <Card className="rounded-lg border-primary/20 bg-card shadow-sm h-fit sticky top-4">
            <CardHeader className="border-b border-border/50 bg-muted/10 pb-4">
              <CardTitle className="text-base flex items-center justify-between">
                <span>
                  {isComparing ? selectedBroker.name : "Charge Summary"}
                </span>
              </CardTitle>
            </CardHeader>
            ...
            <Card className="rounded-lg border-amber-500/30 bg-card shadow-sm h-fit sticky top-4">
              <CardHeader className="border-b border-border/50 bg-muted/10 pb-4">
                <CardTitle className="text-base flex items-center justify-between text-amber-600 dark:text-amber-400">
                  ...
                  <span className="text-xs px-2 py-1 bg-amber-500/10 rounded-md font-bold">
                    Comparison
                  </span>
                </CardTitle>
              </CardHeader>
